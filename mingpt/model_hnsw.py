...
class GPTConfig:
  ...
  Attributes:
    ...
    n_embd: Embedding dimension for transformer
    node_embd: Embedding dimension for node IDs (smaller, since node IDs are random indices)
  ...
  resid_pdrop = 0.1
  attn_pdrop = 0.1
  node_embd = 32  # Default smaller embedding for node IDs
...
class GPT(nn.Module):
  ...
  def __init__(self, config):
    ...
    node_embd = getattr(config, 'node_embd', 32)
    self.node_embd = node_embd

    # Node ID embedding (~1M nodes) - shared for both state nodes and actions
    self.node_embeddings = nn.Embedding(config.vocab_size, node_embd)

    # k and r conditioning embeddings (small vocab, use node_embd dimension)
    # k: -1 to max_k (add 2 for offset: -1 maps to 0, 0 maps to 1, etc.)
    self.k_emb = nn.Embedding(config.max_k + 2, node_embd)
  ...
  def forward(self, query_vectors, node_ids, k_values, r_values, actions=None, targets=None, mask=None):
    ...
    # Get raw embeddings (no projection yet)
    query_emb = query_vectors.type(torch.float32)  # (batch, seq_len, 128)
    node_emb = self.node_embeddings(node_ids.type(torch.long))  # (batch, seq_len, node_embd)

    # Embed k and r conditioning: (batch, node_embd)
    # k values are shifted by 1 so -1 maps to index 0
    k_emb = self.k_emb((k_values + 1).type(torch.long))  # (batch, node_embd)
    r_emb = self.r_emb(r_values.type(torch.long))  # (batch, node_embd)

    # Expand k/r to match sequence length
    k_cond = k_emb.unsqueeze(1).expand(-1, seq_len, -1)  # (batch, seq_len, node_embd)
    r_cond = r_emb.unsqueeze(1).expand(-1, seq_len, -1)  # (batch, seq_len, node_embd)

    # Concatenate all components and project to n_embd
    # [query(128), node(32), k(32), r(32)] -> n_embd
    state_concat = torch.cat([query_emb, node_emb, k_cond, r_cond], dim=-1)
    state_emb = self.state_proj(state_concat)  # (batch, seq_len, n_embd)
    ...
    if actions is not None:
      ...
      # Reuse node_embeddings for actions (they're the same node IDs)
      action_emb = self.action_proj(self.node_embeddings(actions_for_emb.type(torch.long)))
    ...
    # Compute loss and accuracy if targets provided
    loss = None
    acc = None
    if targets is not None:
      ...
      # Compute accuracy on valid (non-padded) positions
      with torch.no_grad():
